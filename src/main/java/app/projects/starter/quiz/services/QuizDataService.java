...
public class QuizDataService {
    ...
    static Map<Difficulty, Integer> calculateEachDifficultyQuestionCount(int numberOfQuestions, Difficulty difficulty, CategoryQuestionCountInfoDto categoryQuestionCount) {
     ...
     int missingQuestions = numberOfQuestions - eachDifficultyQuestionCount.values().stream()
             .mapToInt(i -> i).sum();
     ...
     while (missingQuestions > 0) {
         ...
         if (closestDifficulty == null) {
             log.warning("Not enough question in given category!" + eachDifficultyQuestionCount);
             break;
         }
         ...
         missingQuestions = numberOfQuestions - eachDifficultyQuestionCount.values().stream()
                 .mapToInt(i -> i).sum();
     }
